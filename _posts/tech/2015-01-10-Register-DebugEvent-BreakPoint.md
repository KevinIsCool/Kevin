...
{% endhighlight%}     
这其中包含了指令地址、指令操作码和高级的汇编指令。为了在这个地址处设置一个软断点，使CPU可以在此处暂定执行，我们需要从双字节的操作码`8BC3`处置换一个字节出来。需要用于替换的字节则是一个`INT3`中断指令，这个指令将高速CPU暂定执行当前的进程。`INT3`指令的操作码为一个单字节值`0xCC`。以上一条指令为例，设置了断点前后的状态为：     
**设置软断点之前的操作码**   
{% highlight C %}
0x44332211:    8BC3    MOV EAX, EBX
{% endhighlight%}     
